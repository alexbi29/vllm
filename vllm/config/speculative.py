        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                # deepseek-ai/dspark_qwen3_8b_block7
                    ...
                    or getattr(
                        self.draft_model_config.hf_config, "dspark_block_size", 0
                    )
                    or (
                ...
                if self.method == "dspark" and self.num_speculative_tokens is None:
                    self.num_speculative_tokens = getattr(
                        self.draft_model_config.hf_config, "dspark_block_size", None
                    )
                if (
                    self.method == "dspark"
                ...
                if n_predict is not None:
                    if self.num_speculative_tokens is None:
                        ...
                        # MTP reuses prediction layers cyclically. DSpark drafts
                        # its token block in parallel, independently of depth.
                        raise ValueError(
