...
def get_max_concurrency_for_kv_cache_config(
    ...
    Only the steady part of each pool's per-request footprint scales with
    ...
    Host-resident groups use a separate pool, so the smaller pool-specific
    concurrency limit applies.
    """
    limits: list[float] = []
    for host_resident, capacity in (
        (False, kv_cache_config.num_blocks),
        (True, kv_cache_config.hisparse_host_num_blocks),
    ):
        groups = [
            group
            for group in kv_cache_config.kv_cache_groups
            if group.host_resident == host_resident
        ]
        if not groups:
            continue
        ...
        if steady:
            limits.append(max(capacity - transient, 0) / steady)
    return min(limits)
...
def get_kv_cache_configs(
    ...
    if compact_aggregate_signature is not None:
        for kv_cache_config in kv_cache_configs:
            ...
            if (
                vllm_config.cache_config.get_resolved_kv_cache_layout().is_block_outermost
