    ...
    *blocks_per_chunk*, *compact_cpu_budget_bytes*, and *compact_page_size*
        ...
        if self._compact_enabled:
            ...
            for key in keys:
                ...
                assert chunk.is_ready, (
                    f"Chunk {key!r} is not ready for reading (pending)"
                )
                if chunk.ref_cnt == 0:
    ...
    def _compact_prepare_store(
        ...
        if not allocator.simulate_batch_allocation(sizes):
            ...
            def can_fit(candidates: list[tuple[OffloadKey, ChunkStatus]]) -> bool:
        ...
        for key, alloc, group_idx in allocated_allocations:
            ...
            # Insert into LRU/ARC policy with a dummy chunk so evict_until
            # can track this key for future eviction ordering.
            self._policy.insert(key, ChunkStatus(chunk_id=0))
        ...
        if success:
            for key in keys:
                ...
                if alloc is not None:
                    ...
                    stored_keys.append(key)
                    # Mark the policy dummy chunk ready and evictable.
                    chunk = self._policy.get(key)
                    if chunk is not None and not chunk.is_ready:
                        chunk.ref_cnt = 0
                        self._num_write_pending_chunks -= 1
