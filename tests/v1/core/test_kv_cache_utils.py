...
def test_dsv4_flash_0731_kv_capacity(monkeypatch):
    ...
    # Use the integration tree's public grouping entry point; the older
    # private grouping helpers used by the original PR have been removed.
    groups = get_kv_cache_groups(vllm_config, specs)
    assert len(groups) == 5
    compressor_counts = sorted(
        len(g.layer_names) for g in groups if "compressor" in g.layer_names[0]
    )
    ...
    # This integration's packed layout has a different shared block stride
    # from the PR's original fixture. Use a pool that fits steady + transient;
    # otherwise concurrency is below one and the new estimate can be smaller.
    available = 16 * GiB_bytes
